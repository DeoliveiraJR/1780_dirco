...
- As linhas variáveis podem carregar flags por mês:
...
- O contexto moderno também deve carregar `serie_historica` por referência quando a fórmula usa janela temporal
- O contexto deve preservar a diferença entre mes vazio e `0` explicito
...
- _extrair_janela_historica_por_periodo
- _extrair_sazonalidade_historica
...
- `0` digitado deve manter a célula como preenchida; célula limpa deve voltar a ser vazio lógico
...
6. Confirmar se a divergência é entre mês vazio e `0` explícito na linha de referência
...
### Sintoma: Excel ignora um mês, mas a DRE não
Checklist:
1. Verificar se o mês foi persistido como vazio ou como `0`
2. Conferir `projetado_preenchido` / `valores_preenchidos` na linha envolvida
3. Confirmar se a `serie_historica` do contexto removeu meses vazios do ano corrente
4. Validar se a função nativa consumida ignora vazios (`MEDIA`, `MEDIA_INTERNA`, `MINIMO`, `MAXIMO`, `DESVIO_PADRAO`, `SOMA`)
